...
class Visitor {
  ...
  constructor(option = {}) {
    ...
    this.config.clientPath = path.join(this.outputDir, this.config.package, `${_snakeCase(this.config.clientName, this.config.keywords)}.py`);
...
setup(
  ...
  visitInit(ast, types, level) {
    ...
    types.forEach((item) => {
      ...
      this.visitComments(comments, level + 2);
      this.emit(`${_snakeCase(_vid(item.vid), this.config.keywords)}: `, level + 2);
    ...
    if (ast.params.params.length !== 0) {
      ...
      for (var i = 0; i < ast.params.params.length; i++) {
        ...
        const name = _avoidKeywords(_snakeCase(_name(node.paramName), this.config.keywords));
      ...
      for (let i = 0; i < imports.length; i++) {
        ...
        if (!moduleDir && innerPath) {
          ...
          if (pyPath.startsWith('./') || pyPath.startsWith('../')) {
            pyPath = pyPath.split('/').map(dir => _snakeCase(dir, this.config.keywords)).join(path.sep);
          ...
          const filename = _snakeCase(path.basename(pyPath, '.py'), this.config.keywords);
        ...
        let filename = _snakeCase(className, this.config.keywords);
        ...
        if (inner && pkg.exports[inner]) {
          ...
          arr.map(key => {
            classNamespace += '.' + _snakeCase(key, this.config.keywords);
          });
  ...
  visitModelBody(ast, level, modelName) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      this.visitComments(comments, level);
      let param = _avoidKeywords(_snakeCase(_escape(_name(node.fieldName)), this.config.keywords));
  ...
  visitModel(modelBody, modelName, extendOn, level, filename) {
    ...
    this.saveBuffer(path.join(this.modelPath, filename || `_${_snakeCase(modelName, this.config.keywords)}.py`));
    this.usedTypes = [];
    this.imports = [];
    this.output = '';
  }
    ...
    if (ast.extendFileds && ast.extendFileds.length > 0) {
      ...
      if (baseErr) {
        ...
        for (let i = 0; i < ast.extendFileds.length; i++) {
          ...
          let value = _snakeCase(_escape(_name(node.fieldName)), this.config.keywords);
  ...
  visitException(exceptionBody, exceptionName, extendOn, level) {
    ...
    this.saveBuffer(path.join(this.exceptionPath, `_${_snakeCase(exceptionName, this.config.keywords)}.py`));
    this.usedTypes = [];
    this.imports = [];
    this.output = '';
  }
  ...
  eachSubModel(ast, level) {
    ...
    const modelName = _subModelName(_name(ast.modelName));
    const mainModelName = _name(ast.modelName).split('.')[0];
    this.visitModel(ast.modelBody, modelName, ast.extendOn, level, `_${_snakeCase(mainModelName, this.config.keywords)}.py`);
  }
  ...
  visitMethodCall(ast, level) {
    ...
    const functionName = _avoidKeywords(_snakeCase(name, this.config.keywords));
    ...
    if (name.startsWith('$') && this.builtin[name]) {
      ...
      const functionName = _snakeCase(name, this.config.keywords);
  ...
  visitInstanceCall(ast, level) {
    ...
    if (ast.builtinModule && this.builtin[ast.builtinModule] && this.builtin[ast.builtinModule][method]) {
      ...
      if (ast.left.id.tag === DSL.Tag.Tag.VID) {
        this.emit(`self.${_snakeCase(_vid(ast.left.id), this.config.keywords)}`);
      } else {
        const aliasId = _snakeCase(_name(ast.left.id), this.config.keywords);
        this.emit(aliasId);
      }
      this.emit(`.${(_avoidKeywords(_snakeCase(method, this.config.keywords)))}`);
      if (async) {
        this.emit('_async');
      }
      this.visitArgs(ast.args, level);
  ...
  visitStaticCall(ast, level) {
    ...
    const functionName = _avoidKeywords(_snakeCase(_name(ast.left.propertyPath[0]), this.config.keywords));
  ...
  visitPropertyAccess(ast, env = {}) {
    ...
    var id = _snakeCase(_name(ast.id), this.config.keywords);
    if (ast.id.tag === Tag.VID) {
      id = `self.${_snakeCase(_vid(ast.id), this.config.keywords)}`;
    }
    ...
    for (var i = 0; i < ast.propertyPath.length; i++) {
      ...
      if (current.type === 'model') {
        expr += `.${_avoidKeywords(_snakeCase(name, this.config.keywords))}`;
      } else {
  ...
  visitExpr(ast, level, env) {
    if (ast.type === 'boolean') {
      ...
      if (ast.inferred && ast.inferred.type === 'basic' && ast.inferred.name === 'class') {
        ...
        this.emit(_avoidKeywords(_snakeCase(_name(ast.id), this.config.keywords)));
      }
    } else if (ast.type === 'virtualVariable') {
      this.emit(`self.${_snakeCase(_vid(ast.vid), this.config.keywords)}`);
  ...
  visitMapAccess(ast, level, env = {}) {
    ...
    if (ast.id.tag === DSL.Tag.Tag.VID) {
      expr = `self.${_snakeCase(_vid(ast.id), this.config.keywords)}`;
    } else {
      expr = `${_avoidKeywords(_snakeCase(_name(ast.id), this.config.keywords))}`;
    }
    const prefix = env.left ? '[' : '.get(';
    const suffix = env.left ? ']' : ')';
  ...
  visitFor(ast, level) {
    ...
    if (ast.list.inferred && ast.list.inferred.itemType
      ...
      this.emit(`${_avoidKeywords(_snakeCase(_name(ast.id), this.config.keywords))} in `);
      this.visitExpr(ast.list, level);
    }
    this.emit(':\n');
    this.visitStmts(ast.stmts, level + 2);
  ...
  visitAssign(ast, level) {
    if (ast.left.type === 'property_assign' || ast.left.type === 'property') {
      ...
      this.emit(`self.${_snakeCase(_vid((ast.left.vid)), this.config.keywords)}`, level);
    } else if (ast.left.type === 'variable') {
      this.emit(`${_avoidKeywords(_snakeCase(_name(ast.left.id), this.config.keywords))}`, level);
  ...
  visitDeclare(ast, level) {
    var id = _avoidKeywords(_snakeCase(_name(ast.id), this.config.keywords));
    this.emit(`${id}`, level);
    if (id === 'new_full_str') {
      this.emit('');
    }
  ...
  eachFunction(ast, level) {
    ...
    // let comments = DSL.comment.getFrontComments(this.comments, ast.tokenRange[0]);
    // this.visitComments(comments, level);
    const functionName = _avoidKeywords(_snakeCase(_name(ast.functionName), this.config.keywords));
  ...
  eachAPI(ast, level) {
    ...
    const apiName = _avoidKeywords(_snakeCase(_name(ast.apiName), this.config.keywords));
    this.isStaticFunction = false;
  ...
  getModelName(name, moduleName, type = 'model') {
    ...
    if (type === 'model' && moduleName) {
      ...
      const aliasName = `${_snakeCase(moduleName.replace(/-/g, ''), this.config.keywords)}_models`;
      this.imports.push({
        aliasName,
    ...
    if (type === 'exception' && moduleName) {
      ...
      const aliasName = `${_snakeCase(moduleName.replace(/-/g, ''), this.config.keywords)}_exceptions`;
      this.imports.push({
        aliasName,
        packageName: namemespace,
