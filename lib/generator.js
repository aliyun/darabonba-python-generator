...
setup(
  ...
  visitToMapField(fieldValue, fieldName, key, value, level, arr = false) {
    ...
    fieldValue.fieldType.idType === 'model' || fieldValue.fieldType.idType === 'builtin_model'))) {
      ...
      this.visitToMapField(fieldValue.fieldItemType || fieldValue.subType, fieldName, `${key}`, `k${deep}`, level + 2, true);
    } else if (fieldValue.fieldType === 'map' || fieldValue.type === 'map') {
      this.emit(`${key} = {}\n`, level);
      this.emit(`for k${deep}, v${deep} in self.${fieldName}.items():\n`, level);
      this.visitToMapField(fieldValue.valueType, fieldName, `${key}[k${deep}]`, `v${deep}`, level + 2);
    } else {
  ...
  visitToMap(ast, level) {
    ...
    for (let i = 0; i < ast.nodes.length; i++) {
      ...
      if(!this.checkFieldHasModel(node.fieldValue)) {
        ...
        this.visitToMapField(node.fieldValue, param, `result['${realName}']`, `self.${param}`, level + 4);
      }
      this.emit('\n');
    }
  ...
  visitFromField(fieldValue, fieldName, key, value, level, arr = false) {
    ...
    if (fieldValue.type === 'modelBody') {
      const subModelName = _subModelName(_camelCase(fieldName));
      ...
      this.emit(`${key} = []\n`, level);
      this.emit(`for k${deep} in ${value}:\n`, level);
      this.visitFromField(fieldValue.fieldItemType || fieldValue.subType, fieldName, `${key}`, `k${deep}`, level + 2, true);
    } else if (fieldValue.fieldType === 'map' || fieldValue.type === 'map') {
      this.emit(`${key} = {}\n`, level);
    ...
    (fieldValue.fieldType && fieldValue.fieldType.type === 'moduleModel')) {
      ...
      this.emit(`temp_model = ${this.getModelName(modelName, _name(moduleId), 'module')}()\n`, level);
      this.emitFieldConvert(key, `temp_model.from_map(${value})`, level, arr);
      ...
      this.emit(`temp_model = ${modelName}()\n`, level);
      this.emitFieldConvert(key, `temp_model.from_map(${value})`, level, arr);
