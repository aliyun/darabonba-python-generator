...
const {
  _name, _vid, _string, _upperFirst, _lowerFirst,
  _subModelName, _avoidKeywords, _camelCase, _snakeCase,
  _deepClone, _isSnakeCase, _isBasicType, _isBinaryOp, _isBuiltinModel,
  CORE, REQUEST, RESPONSE, EXCEPTION, RESP_EXCEPTION, MODEL
} = require('./helper');
...
class Generator {
  constructor(option = {}) {
    ...
    this.config.clientPath = path.join(this.outputDir, this.config.package, `${_snakeCase(this.config.className)}.py`);
    this.used = [];
  ...
  saveProjectFiles(packageInfo, requirePackage) {
  ...
  getInnerClient(aliasId, pyPath) {
    ...
    const fileInfo = path.parse(pyPath);
  ...
  saveInnerModule(ast, targetPath) {
    ...
    while (!data.done) {
      ...
      const filepath = path.join(path.dirname(targetPath), `${ast.innerModule.get(aliasId)}.py`);
  ...
  save(filepath) {
    ...
    const content = `# This file is auto-generated, don't edit it. Thanks.
...
${this.output}`;
  ...
  visitExceptions(ast, filepath, level) {
    ...
    for (let i = 0; i < exs.length; i++) {
      ...
      const modelFilepath = path.join(exDir, `${exceptionName}.py`);
      ...
      if (this.predefined) {
        Object.keys(this.predefined).filter((key) => {
          ...
          const subModelFilepath = path.join(modelDir, `${realFullModelName.split('\\').join(path.sep)}.py`);
  ...
  visitModels(ast, filepath, level) {
    ...
    for (let i = 0; i < models.length; i++) {
      ...
      const modelFilepath = path.join(modelDir, `${modelName}.py`);
      const classNamespace = this.classNamespace.get(filepath);
  ...
  getClassNamespace(pyPath) {
    if (pyPath.startsWith(this.outputDir)) {
      const baseDir = path.join(this.outputDir, 'src', path.sep);
      pyPath = pyPath.replace(baseDir, '');
    }

    const arr = pyPath.split(path.sep).slice(0, -1);
    ...
    if (imports.length > 0) {
      ...
      for (let i = 0; i < imports.length; i++) {
        ...
        if (!moduleDir && innerPath) {
          let pyPath = innerPath.replace(/(\.tea)$|(\.spec)$|(\.dara)$/gi, '');
          if (pyPath.startsWith('./') || pyPath.startsWith('../')) {
            pyPath = path.join(path.dirname(filepath), `${pyPath}.py`);
        ...
        const pyRelease = pkg.releases && pkg.releases.py;
        const pyPkg = pkg.py;
        if (!pyRelease || !pyPkg || !pyPkg.package) {
          throw new Error(`The '${aliasId}' has no py supported.`);
        }
        const classNamespace = pyPkg.package.split('.').join('\\');
        const className = pyPkg.clientName || 'Client';
        const [key, value] = pyRelease.split(':');
        ...
        this.moduleTypedef[aliasId] = pyPkg.typedef;
      }
