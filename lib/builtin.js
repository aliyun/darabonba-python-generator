...
class Converter {
  ...
  readable(ast) {
    const expr = ast.args[0];
    this.generator.usedTypes.push('StringIO');
    this.generator.imports.push({
      className: 'json',
    });
    this.generator.emit('StringIO(json.dumps(');
    this.generator.visitExpr(expr);
    this.generator.emit('))');
  }
  ...
  writable(ast) {
    const expr = ast.args[0];
    this.generator.usedTypes.push('BytesIO');
    this.generator.imports.push({
      className: 'json',
    });
    this.generator.emit('BytesIO(json.dumps(');
    this.generator.visitExpr(expr);
    this.generator.emit(').encode(\'utf-8\'))');
  }
}
